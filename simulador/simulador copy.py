...
# --- GRÁFICO DE SENSIBILIDADE: SPREAD ANUAL POR DELAY ---
st.subheader("⏰ Sensibilidade do Spread Anual ao Delay (Tempo de Desemprego)")
...
for delay_iter in delay_range:
    ...
    try:
        ...
        st.error(f"Erro ao calcular sensibilidade para delay de {delay_iter} meses: {e}")
...
if spread_atual_delay_marker is not None and not np.isnan(spread_atual_delay_marker):
    fig_delay.add_trace(
        go.Scatter(
            ...
            mode='markers+text',
            marker=dict(size=16, color='darkorange', symbol='diamond'),
...
fig_delay.update_layout(
    title="Sensibilidade do Spread Anual ao Delay (Tempo de Desemprego Esperado)",
...
for var in variables_tornado:
    ...
    # Se a variável for inteira (n ou delay), arredondar
    if var['label'].startswith('Número') or var['label'] == 'Delay':
