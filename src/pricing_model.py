...
def simulate_pricing(params):
    ...
    Args:
        params (dict): Dicionário contendo PV, n, p_close_annual, p_rot_m, delay, p_obito_ann
    ...
    d = int(params['delay'])
    p_obito_ann = float(params.get('p_obito_ann', 0.0))
    if n > 120:
    ...
    f_def = np.zeros(n+1); f_del = np.zeros(n+1); f_death = np.zeros(n+1)
    ...
    def epv_components(PMT):
        ...
        # EPV óbito (perda parcial de saldo com LGD_DEATH)
        death_pv = sum(
            f_death[u] * (
                sum(PMT / (1 + r_tar_m)**t for t in range(1, u)) +
                (1 - LGD_DEATH) * bal[u-1] / (1 + r_tar_m)**u
            )
            for u in range(1, n+1)
        )
        return surv_pv, delay_pv, def_pv, death_pv

    def epv_minus_pv(PMT):
        surv_pv, delay_pv, def_pv, death_pv = epv_components(PMT)
        return (surv_pv + delay_pv + def_pv + death_pv) - PV

    with warnings.catch_warnings():
        warnings.filterwarnings('ignore')
        PMT_risco = fsolve(epv_minus_pv, PMT_base)[0]
    surv_pv, delay_pv, def_pv, death_pv = epv_components(PMT_risco)
    ...
    LGD_ponderado_death = (f_death[1:] * (LGD_DEATH * bal[:-1])).sum() / PV
    ...
    return {
        ...
        'h_default': h_default, 'h_delay': h_delay, 'h_death': h_death,
        'S_n': S_n,
        'P_default_total': P_default_total, 'P_delay_total': P_delay_total, 'P_death_total': P_death_total,
        'EPV_surv': surv_pv, 'EPV_delay': delay_pv, 'EPV_default': def_pv, 'EPV_death': death_pv,
        'Expected_payments': Expected_payments,
        'Mean_time_to_event': Mean_time_to_event,
        'E_Duration': E_Duration,
        'LGD_ponderado': LGD_ponderado, 'LGD_ponderado_death': LGD_ponderado_death,
